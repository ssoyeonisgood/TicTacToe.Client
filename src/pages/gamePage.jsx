  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-slate-50 p-6">
      <div className="w-full max-w-2xl flex justify-between items-center mb-6">
        <div>
          <h2 className="text-lg font-semibold">Room: <span className="font-mono">{roomCode}</span></h2>
          <p className="text-sm text-gray-500">Player X: {game.playerX || "-" } · Player O: {game.playerO || "-"}</p>
        </div>
        <div className="flex gap-2 items-center">
          <button onClick={copyCode} className="px-3 py-1 bg-indigo-600 text-white rounded">Copy Code</button>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded shadow">
        ...
        <div className="mt-4 text-center">
          {game.isFinished ? (
            ...
            <div className="text-lg">Turn: <span className="font-mono">{game.currentTurn}</span></div>
          )}
        </div>
      </div>
    </div>
